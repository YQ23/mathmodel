function [label, alpha, mu, sigma] = Gaussian_Mixture(data, k, iter)
...
% Initialization
alpha = ones(k, 1) / k;
mu = data(randperm(size(data,1),k)',:);
for i = 1 : k
    sigma{i, 1} = rand * eye(size(data, 2));
end
label = zeros(size(data, 1), 1);

p = Posteriori(data, k, alpha, mu, sigma); % calculate posteriori probability
...
for i = 1 : iter
    ...
    for j = 1 : k
        ...
        for num = 1 : size(data, 1)
            tsigma  = tsigma + p(num, j) * (data(num, :) - mu(j, :))' * (data(num, :) - mu(j, :));
        end
    ...
    % E-Step
    p = Posteriori(data, k, alpha, mu, sigma);
end
[~, label] = max(p,[], 2); % classification
end

function p = Posteriori(data, k, alpha, mu, sigma)
